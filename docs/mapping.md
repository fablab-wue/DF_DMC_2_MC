...
On SliderMC: bare `PG` plays the whole buffer; `PG <start> <end>` is **0-based inclusive** sample indices. `start > end` plays **reverse** with negated deltas. `PI` replies `PI:<play_index>`. See [motion-path.md](https://github.com/fablab-wue/SliderDoc/blob/main/mc/motion-path.md).
